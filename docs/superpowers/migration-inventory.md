...
| 58 | poster | 工作 | 新建词根卡 | poster | 名词 | —（外来词） | — |
...
| 65 | merintis | 教会 | 并入 rintis | rintis（补建） | — | — | — |
...
| 82 | darat | 地形 | 新建词根卡 | darat | 名词 | mendarat（着陆） | — |
...
| 113 | sebab | 语法 | 新建词根卡 | sebab | 连词 | — | oleh sebab itu |
| 114 | dibentuk | 词缀 | 并入 bentuk | bentuk（补建） | — | — | — |
| 115 | cerita | 高频 | 新建词根卡 | cerita | 名词 | menceritakan（讲述、叙述）/ bercerita（讲故事） | — |
...
| 146 | tegur | 高频 | 新建词根卡 | tegur | 动词根 | menegur（责备、提醒）/ teguran（责备，名词） | — |
...
| 150 | derita | 教会 | 新建词根卡 | derita | 名词 | menderita（受苦）/ penderitaan（苦难） | — |
...
| 163 | surat tugas | 工作 | 新建词组卡 | surat tugas | — | — | — |
...
| 175 | -an（约数） | 词缀 | 新建词根卡 | -an | 词缀（不单用）※例外，见下方说明 | — | — |
...
| 189 | macam | 高频 | 新建词根卡 | macam | 名词 | bermacam-macam（各式各样）/ semacam（一种、类似于） | — |
...
| 221 | cair | 工作 | 新建词根卡 | cair | 形容词 | mencairkan（拨款、提现；使融化）/ pencairan（放款、提现流程） | — |
| 222 | pencairan | 工作 | 并入 cair | cair（#221） | — | — | — |
| 223 | kwitansi | 工作 | 新建词根卡 | kwitansi | 名词 | —（外来词） | — |
| 224 | nota | 工作 | 新建词根卡 | nota | 名词 | —（外来词） | — |
| 225 | faktur | 工作 | 新建词根卡 | faktur | 名词 | —（外来词） | faktur pajak |
| 226 | pajak | 工作 | 新建词根卡 | pajak | 名词 | perpajakan（税务领域） | — |
| 227 | modal | 工作 | 新建词根卡 | modal | 名词 | bermodal（有本钱的） | — |
| 228 | untung | 工作 | 新建词根卡 | untung | 名词 | beruntung（幸运的）/ keuntungan（利润；好处） | — |
...
| 257 | memikul | 教会 | 并入 pikul | pikul（补建） | — | — | memikul salib |
...
| 264 | mengerikan | 词缀 | 并入 ngeri | ngeri（补建） | — | — | — |
...
`#175 -an（约数）` 原卡讲的是后缀本身，不是词。两种卡片类型都套不上：它不是裸词根，
也不是固定搭配。本轮按 `新建词根卡` 处理，`w` 写 `-an`，`词性` 写 `词缀（不单用）`——
这个取值**不在**设计文档的词性标注表里，是这一条独有的例外。迁移时若决定不保留这类
纯语法卡，改判 `丢弃`、把内容并进别的卡的 `syn` 即可，但要先改这张表。
...
从 coll 抽出的 38 张（去重后；`berat hati`、`di samping itu`、`apa saja`、`hawa nafsu`
四处重复提及已合并，不另立卡）：
