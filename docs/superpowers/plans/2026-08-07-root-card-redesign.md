...
- 界面文案全部中文，沿用现有语气。
- **迁移时只改硬错误，用户写的其余内容一律保留。** 可以改的：谐音标错音、例句是残句、
  事实性错误（缩写全称错、词源错、语域错）。不可以改的：句子偏短、换个说法更顺、
  自己选的经文/例句更贴切——这些是风格判断，不是缺陷。删掉一条看不出用意的提示前，
  先去别的卡上找它的另一半：用户常把易混词对写在两张卡上，只删一边会让词库变成单向警告。
